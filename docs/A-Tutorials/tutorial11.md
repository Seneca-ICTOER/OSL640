...
# Tutorial 11: Sed and Awk Utilities
...
**ATTENTION: The due date for successfully completing this tutorial (i.e. tutorial 11) is by Friday, August 16 @ 11:59 PM (Week 14).**
...
```bash
cp ~osl640/tutorial11/cars2.txt .
```
...
1. `sed -n '3,6 p' ~osl640/stuff.txt`
2. `sed '4 q' ~osl640/stuff.txt`
...
1. Write a Linux sed command to display only lines 5 to 9 for the file: **~osl640/stuff.txt**
2. Write a Linux sed command to display only lines the begin the pattern “and” for the file: **~osl640/stuff.txt**
...
1. `awk ‘NR == 3 {print}’ ~osl640/stuff.txt`
2. `awk ‘NR >= 2 && NR <= 5 {print}’ ~osl640/stuff.txt`
3. `awk ‘$1 ~ /This/ {print $2}’ ~osl640/stuff.txt`
