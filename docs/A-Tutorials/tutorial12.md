...
- Week 12 Lecture 1 Notes: [PDF](https://matrix.senecacollege.ca/~osl640/slides/OSL640-Week12.pdf) \| [PPTX](https://matrix.senecacollege.ca/~osl640/slides/OSL640-Week12.pptx)
...
_The **while** loop is useful to loop based on the result from a test condition or command result. This type of loop is very useful for **error-checking**._
...
**ATTENTION: The due date for successfully completing this tutorial (i.e. tutorial 12) is by Friday, April 21 @ 11:59 PM (Week 14).**
...
```bash
./exit.bash osl640 Linux 2> error.txt
```
...
```bash
./break-2.bash nwk680 prg600 osl640 osm620
```

   - What do you notice? How come **osl640** and **osm620** are NOT displayed but a message appeared at the end of the script that the script completed?

  13. Issue the following to run a checking script:

```bash
~osl640/week12-check-3
```
...
Here is a link to the [MS Word Document of ALL of the questions](https://matrix.senecacollege.ca/~osl640/questions/osl640_week12_practice.docx) displayed below but with extra room to answer on the document to simulate a quiz
