...
sidebar_position: 3
description: Tutorial 3 - Advanced File Management and Quoting Special Characters
---
...
- Week 3 Lecture 1 Notes: [PDF](https://matrix.senecacollege.ca/~osl640/slides/OSL640-Week3.pdf) \| [PPTX](https://matrix.senecacollege.ca/~osl640/slides/OSL640-Week3.pptx)
...
- /home/userid/osl640/cars.txt **(absolute pathname)**
...
**ATTENTION: The due date for successfully completing this tutorial (i.e. tutorial 3) is by Friday, March 10 @ 11:59 PM (Week 8).**
...
```
   ...
   - Let's **remove** this directory structure, and issue the same command using a _relative-to-home_ pathname instead.
...
```
  ...
  5. Issue a Linux command to confirm you are located in the _examples_ directory.
...
![Dir12](/img/Dir12.png)
  ...
  3. Issue the **touch** command to create the following empty text files in the _examples_ directory: (note _upper_ and _lowercase_ letters)
...
```
  ...
  5. Issue the **ls** command to get a listing of files in your _examples_ directory.
...
```
   ...
   - **Write down the expected output on paper**, then **issue the command**. Did ALL text files get listed? Why not?
...
```
   ...
   - **Write down the expected output on paper**, then **issue the command**. Did ALL text files get listed this time? If so, why?
...
Here is a link to the [MS Word Document of ALL of the questions](https://matrix.senecacollege.ca/~osl640/questions/osl640_week3_practice.docx) displayed below but with extra room to answer on the document to simulate a quiz
