...
When converting **binary** numbers to **decimal** numbers, perform the following steps:
  ...
  3. Starting on the _rightmost_ "**L**", multiply the value (placeholder) by **2** to the power of zero. 
...
When converting **decimal** numbers to **binary** numbers, perform the following steps:
  ...
  2. On the _right-side_, write the number **1** and moving **leftwards**, keep doubling the numbers until that number is **greater than** the decimal number to be converted (refer to the diagram below).
...
Users that own those _directories_ and _regular files_ are referred to as **users**, users that belong within the same group are referred to as same group members, and those users that do NOT belong to a particular group are referred to as **other group members**.
...
**ATTENTION: The due date for successfully completing this tutorial (i.e. tutorial 4) is by Friday, March 8 @ 11:59 PM (Week 8).**
...
![Calc Check](/img/Calc-check.png)
  ...
  4. Perform a manual conversion of the **decimal number 55** to a **8-bit binary number**.

      - What method (displayed above) will you use? Use a calculator to check your work.

  5. Perform a manual conversion of the **octal number 461** to a **8-bit binary number**.
  ...
  7. Perform a manual conversion of the **hexadecimal number ABC** to a **8-bit binary number**.
...
```
  ...
  10. Issue the **ls -ld** and **ls -lR** commands (as you did in _step \#8_) to confirm that the permissions for those directories have been changed.
