...
- Week 6 Lecture 1 Notes: [PDF](https://matrix.senecacollege.ca/~osl640/slides/OSL640-Week6.pdf) \| [PPTX](https://matrix.senecacollege.ca/~osl640/slides/OSL640-Week6.pptx)
...
If you installed the graphical Secure Shell application in [TUTORIAL 1 INVESTIGATION 1](./tutorial1.md#investigation-1-accessing-your-matrix-linux-account), you can run a **graphical** application from your computer desktop to transfer files between your computer and your Matrix account.
...
is **NOT** a _Windows_ machine, you can skip this investigation.

![Graphical SFTP Application](/img/Graphical-sftp-application.png)

**Perform the Following Steps:**

  1. On your local computer, open the `NotePad` application to create a text file.
  2. Enter a few lines of text, and then click on the **File** menu and select **save as** (save as the filename `yourfile.txt` in your `local` directory) and then **exit** the _Notepad_ text editor.
  ...
  5. On your Windows desktop, point and **double-click** on the **SSH Secure file Transfer** application icon.

![Graphical SFTP Icon](/img/Graphical-sftp-icon.png)

  6. The main Secure Shell SFTP application window should appear.

        - **FYI**: This application shows files on your home computer (on the left-side) and files on your remote Matrix computer on the right-side. You will NOT see files for your Matrix server since you have NOT logged into the Matrix server.
  ...
  10. After entering the _hostname_ and _username_, click the **Connect** button.
  11. Enter your Seneca password when prompted.
  12. When you correctly connect to your Matrix account, you should see **folders** in your remote server (i.e. Matrix server).
  13. There are two sub-windows to represent your local computer on the left and the remote (Matrix) server on the right.
  14. In the left (local computer) window, point and **double-click** on your Windows profile name (i.e. _username_) and then locate and **double-click** on the **local** directory.
  15. In the right (remote Matrix server) window, navigate to the **remote2** directory and **double-click** to move to that directory.
  16. In the left window, click and drag the **yourfile.txt** file to the right window.

![Graphical SFTP Application](/img/Graphical-sftp-application.png)

  17. Check to see that the **yourfile.txt** file appears in the right window to confirm that the file has been transferred to your Matrix server.

         - **NOTE**: You can select multiple files by using SHIFT-CLICK or CTRL-CLICK methods and then click and drag file selections between computers.

  18. Take a few moments to noter other buttons for both local and remote windows to **refresh the view**, **delete file(s)**, **navigate up to parent directory**, etc.
...
Here is a link to the [MS Word Document of ALL of the questions](https://matrix.senecacollege.ca/~osl640/questions/osl640_week6_practice.docx) displayed below but with extra room to answer on the document to simulate a quiz
