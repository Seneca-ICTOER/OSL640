...
**ATTENTION: The due date for successfully completing this tutorial (i.e. tutorial 9) is by Wednesday, December 13 @ 11:59 PM (Week 14).**
...
**Perform the Following Steps:**
  ...
  3. Issue the following linux Linux command to **copy** a text file to _your_ **home** directory from the OSL640 home directory:

```bash
cp ~osl640/tutorial9/textfile1.txt .
```
...
```bash
cp ~osl640/tutorial9/numbers1.dat .
```
...
```bash
cp ~osl640/tutorial9/numbers2.dat ~/regexps
```
...
![Other re 1](/img/Other-re-1.png)
  ...
  5. Press the letter `q` to **exit** the _man_ pages for **ls**.
...
```bash
cp ~osl640/tutorial9/large-file.txt .
```
...
```
   ...
   - You should see the pattern "osl640" throughout the text file.

![Other re 2](/img/Other-re-2.png)

   - Entering **/osl640** in the **less** command can display all matches of "**osl640**" throughout the text file.
...
![Other re 3](/img/Other-re-3.png)
   ...
   - Let's issue a command from **LAST LINE** MODE to search and replace **osl640** to **OSL640**.
...
```
   ...
   - **NOTE:** The letter **g** after the replace regular expression represents "**global**" and will replace ALL occurrences of osl640 in the text document (as opposed to replacing the first occurrence for every line).
...
![Other re 4](/img/Other-re-4.png)
   ...
   - In **last line** MODE in the **vi** text editor, issuing a command using regular expressions to convert **osl640** to **OSL640**.

  16. Navigate throughout the text file to confirm that ALL occurrences of **osl640** have been replaced with **OSL640**.
  17. Save changes to your vi editing session and exit by typing the following and pressing ENTER:

`:x`

After you complete the _Linux Practice Questions_ section below to get additional practice.
...
1. `grep plym ~osl640/cars`
2. `grep -i fury ~osl640/cars`
3. `grep “^[m-z]” ~osl640/cars`
4. `grep -i “^[m-z]” ~osl640/cars`
5. `grep -i “^[m-z]” ~osl640/cars 5. grep “3$” ~mosl640/cars`
6. `grep -i “c.*5$” ~osl640/cars`
...
1. `grep "^[-+]" ~osl640/tutorial9/numbers.txt`
2. `grep "^[-+]*.[0-9]" ~osl640/tutorial9/numbers.txt`
3. `grep "^[+-]?[0-9]" ~osl640/tutorial9/numbers.txt`

   - (Why?)
   
4. `egrep "^[+-]?[0-9]" ~osl640/tutorial9/numbers.txt`
5. `egrep "^[+-]?[0-9]+$" ~osl640/tutorial9/numbers.txt`
6. `egrep "^[+-]?[0-9]+[.]?[0-9]+$" ~osl640/tutorial9/numbers.txt`
